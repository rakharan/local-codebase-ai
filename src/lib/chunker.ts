...
// Content that carries no semantic value — skip before metadata extraction
// to avoid polluting Qdrant + wasting embedding budget.
function isNoiseContent(content: string): boolean {
  const trimmed = content.trim()
  if (!trimmed) return true

  // CSS hex colors: #ffcc00 !important;
  if (/^#[0-9a-f]{3,8}\s*(!important)?;?$/im.test(trimmed) && !trimmed.includes("\n")) return true

  // CSS properties single-line: margin: 0; padding: 10px;
  if (/^[a-z-]+\s*:\s*[^;{}]+;?\s*(!important)?$/im.test(trimmed) && !trimmed.includes("\n") && !/[a-z]\(|=>|function|class|\bif\b|\bfor\b/i.test(trimmed)) return true

  // Auto-generated boilerplate
  if (/^(\/\/|#|\/\*)\s*(auto-generated|generated by|do not edit|this file was (auto-)?generated)/i.test(trimmed)) return true

  // HTML tags only (single line, no script/php)
  if (/^<\/?[a-z][^>]*>$/i.test(trimmed) && !/script|php|template/i.test(trimmed)) return true

  // Single-line with <5 non-whitespace chars and no logic keywords
  if (!trimmed.includes("\n")) {
    const nonWs = trimmed.replace(/\s/g, "").length
    if (nonWs < 5 && !/\b(function|class|route|select|from|join|insert|update|delete|app\.|rpc|queue|exchange|amqp|cron|env\b|return|throw|catch|await|async|export|import)\b/i.test(trimmed)) return true
  }
...
export function chunkFile(
  ...
  for (let si = 0; si < segments.length; si++) {
    ...
    // skip noise chunks (CSS values, auto-generated boilerplate, HTML tags)
