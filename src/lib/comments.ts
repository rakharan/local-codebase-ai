...
export function extractCommentBlocks(filePath: string, fileContent: string): CommentBlock[] {
  ...
  return blocks.filter(block => block.content.length >= 8 && !isCommentedOutCode(block.content) && !isLowValueComment(block.content))
}

/**
 * Filters comment blocks that carry no semantic knowledge value — CSS values,
 * auto-generated boilerplate, HTML fragments, single-word snippets.
 */
function isLowValueComment(content: string): boolean {
  const trimmed = content.trim()
  if (!trimmed) return true

  // CSS hex colors: #ffcc00 !important;
  if (/^#[0-9a-f]{3,8}\s*(!important)?;?$/im.test(trimmed)) return true

  // CSS properties single-line: margin: 0; padding: 10px;
  if (/^[a-z-]+\s*:\s*[^;{}]+;?\s*(!important)?$/im.test(trimmed) && !trimmed.includes("\n")) return true

  // Auto-generated boilerplate
  if (/^(\/\/|#|\/\*)\s*(auto-generated|generated by|do not edit|this file was (auto-)?generated)/i.test(trimmed)) return true

  // HTML tags only (no prose)
  if (/^<\/?[a-z][^>]*>$/i.test(trimmed) && !/script|php|template/i.test(trimmed)) return true

  // Single-line comments need ≥20 chars or keyword markers to be worth indexing
  if (!trimmed.includes("\n")) {
